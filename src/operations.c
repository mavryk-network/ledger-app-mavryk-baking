 ...
 * @return mv_parser_result: result of the parsing
 */
static mv_parser_result parse_raw_mavryk_header_signature_type(
    raw_mavryk_header_signature_type_t const *const raw_signature_type,
    signature_type_t *signature_type) {
    mv_parser_result res = PARSER_CONTINUE;
 ...
 */
static inline mv_exc compute_pkh(cx_ecfp_compressed_public_key_t *const compressed_pubkey_out,
    ...
    MV_ASSERT_NOT_NULL(compressed_pubkey_out);
    MV_ASSERT_NOT_NULL(contract_out);
    ...
    MV_ASSERT(contract_out->signature_type != SIGNATURE_TYPE_UNSET, EXC_MEMORY_ERROR);
 ...
 * @return mv_parser_result: result of the parsing
 */
static mv_parser_result parse_implicit(
    ...
    out->originated = 0;
    PARSER_CHECK(parse_raw_mavryk_header_signature_type(raw_signature_type, &out->signature_type));
 ...
 * @return mv_parser_result: result of the parsing
 */
static inline mv_parser_result parse_z(uint8_t current_byte,
 ...
 * @return mv_parser_result: result of the parsing
 */
static mv_parser_result parse_next_type(uint8_t current_byte,
 ...
 * @return mv_exc: exception, SW_OK if none
 */
static mv_exc parse_operations_init(struct parsed_operation_group *const out,
                                    bip32_path_with_curve_t const *const path_with_curve,
    ...
    MV_CHECK(compute_pkh((cx_ecfp_compressed_public_key_t *) &out->public_key,
                         &out->signing,
                         path_with_curve));
 ...
 * @return mv_parser_result: result of the parsing
 */
static inline mv_parser_result parse_byte(uint8_t byte,
                                          struct parse_state *const state,
                                          struct parsed_operation_group *const out) {
    mv_parser_result res = PARSER_CONTINUE;
...
#define OP_JMPIF(step, cond)   \
    ...
    switch (state->op_step) {
        ...
        case 0: {
            ...
            // Parse 'source'
            switch (state->tag) {
                // Tags that don't have "originated" byte only support mv accounts, not KT or mv.
                ...
                case OPERATION_TAG_REVEAL: {
                    ...
                    PARSER_CHECK(parse_raw_mavryk_header_signature_type(
            ...
            {
                raw_mavryk_header_signature_type_t const *const sig_type =
                    NEXT_TYPE(raw_mavryk_header_signature_type_t);
                signature_type_t reveal_signature_type = {0};
                PARSER_CHECK(
                    parse_raw_mavryk_header_signature_type(sig_type, &reveal_signature_type));
...
mv_exc parse_operations(buffer_t *buf,
    ...
    MV_CHECK(parse_operations_init(out, path_with_curve, &G.parse_state));

    while (buffer_read_u8(buf, &byte) == true) {
        MV_ASSERT(parse_byte(byte, &G.parse_state, out) != PARSER_ERROR, EXC_PARSE_ERROR);
