/* Mavryk Ledger application - Sign APDU instruction handling
...
static int baking_sign_complete(bool const send_hash) {
    ...
    switch (G.magic_byte) {
        ...
        case MAGIC_BYTE_ATTESTATION:
            MV_CHECK(guard_baking_authorized(&G.parsed_baking_data, &global.path_with_curve));
#ifdef TARGET_NANOS
        ...
        case MAGIC_BYTE_UNSAFE_OP: {
            MV_ASSERT(G.maybe_ops.is_valid, EXC_PARSE_ERROR);
...
int handle_sign(buffer_t *cdata, const bool last, const bool with_hash) {
    ...
    MV_ASSERT(global.path_with_curve.bip32_path.length != 0u, EXC_WRONG_LENGTH_FOR_INS);

    // Guard against overflow
    MV_ASSERT(G.packet_index < 0xFFu, EXC_PARSE_ERROR);
    G.packet_index++;

    // Only parse a single packet when baking
    MV_ASSERT(G.packet_index == 1u, EXC_PARSE_ERROR);
    if (G.packet_index == 1u) {
        CX_CHECK(cx_hash_init_ex((cx_hash_t *) &G.hash_state.state, CX_BLAKE2B, SIGN_HASH_SIZE));
    }

    MV_ASSERT(buffer_read_u8(cdata, &G.magic_byte), EXC_PARSE_ERROR);
    bool is_attestation = false;

    switch (G.magic_byte) {
        case MAGIC_BYTE_PREATTESTATION:
            is_attestation = false;
            MV_ASSERT(parse_consensus_operation(cdata, &G.parsed_baking_data, is_attestation),
                      EXC_PARSE_ERROR);
            break;
        case MAGIC_BYTE_ATTESTATION:
            is_attestation = true;
            ...
            break;
        case MAGIC_BYTE_BLOCK:
            MV_ASSERT(parse_block(cdata, &G.parsed_baking_data), EXC_PARSE_ERROR);
            break;
        case MAGIC_BYTE_UNSAFE_OP:
            // Parse the operation. It will be verified in `baking_sign_complete`.
            MV_CHECK(parse_operations(cdata, &G.maybe_ops.v, &global.path_with_curve));
            break;
        default:
            MV_FAIL(EXC_PARSE_ERROR);
    }
...
static int perform_signature(bool const send_hash) {
    ...
    MV_CHECK(write_high_water_mark(&G.parsed_baking_data));
